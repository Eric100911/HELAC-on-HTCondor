#! /bin/bash
# Main wrapper for full HELAC-Onia workflow (generation + shower)
...
if [ ! -f condor_submit_full.tar ]; then
    echo "Error: condor_submit_full.tar file not found."
    exit 1
fi
...
tar -xvf condor_submit_full.tar

# Check if the script exists
if [ ! -f scripts/helac_full_workflow.sh ]; then
    echo "Error: scripts/helac_full_workflow.sh file not found."
    exit 1
fi

# Load the cmssw-el7 container and run full workflow
cmssw-el7 --command-to-run "bash -x scripts/helac_full_workflow.sh -s $MY_SEED"
...
echo "Full workflow completed successfully with seed $MY_SEED."
