...
cmssw-el7 --command-to-run "bash scripts/helac_build_run.sh -s $MY_SEED"

# Check if the LHE file was created.
if [ ! -f ./test_JpsiJpsiPhi.dat ]; then
    echo "Error: HepMC file test_JpsiJpsiPhi.dat not found."
    exit 1
fi

echo "HELAC-Onia run completed successfully with seed $MY_SEED."
echo "Output HepMC file: test_JpsiJpsiPhi.dat"
...
cp ../../scripts/step1_HepMC_GENSIM.py .
mv ../../test_JpsiJpsiPhi.dat .
# Transfer the HepMC product to output directory after gzip compression
gzip --suffix=.gz --keep --best test_JpsiJpsiPhi.dat
cp test_JpsiJpsiPhi.dat.gz \
    /eos/user/c/chiw/JpsiJpsiPhi/MC_samples/HepMC/DPS-JpsiJpsi-Phi/filter_JPsi_PtMin6p0_Phi_PtMin6p0/DPS-JpsiJpsi-Phi1020_JJPhi_4Mu2K_13p6TeV_TuneCP5_pythia8_${MY_SEED}.dat.gz
# Run CMS simulation step to GENSIM
cmsRun step1_HepMC_GENSIM.py
echo "GENSIM step completed. Output file: step1_Run3Summer22_GENSIM.root"

# Transfer the GENSIM output file back to the output directory
cp step1_Run3Summer22_GENSIM.root \
