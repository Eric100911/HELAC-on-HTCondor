#! /bin/bash
# Main wrapper for shower-only workflow (use existing LHE files)
# This script runs in cmssw-el7 container on HTCondor

set -e

MY_SEED=11
LHE_SOURCE_DIR=${LHE_SOURCE_DIR:-}
LHE_ARCHIVE_DIR=${LHE_ARCHIVE_DIR:-}
...
if [ ! -f condor_submit_shower.tar ]; then
    echo "Error: condor_submit_shower.tar file not found."
    exit 1
fi

# Check if LHE_SOURCE_DIR is set
if [ -z "$LHE_SOURCE_DIR" ]; then
    echo "Error: LHE_SOURCE_DIR environment variable must be set"
    echo "This should point to a directory containing existing LHE files"
    exit 1
fi
...
tar -xvf condor_submit_shower.tar

# Check if the script exists
if [ ! -f scripts/shower_existing_lhe.sh ]; then
    echo "Error: scripts/shower_existing_lhe.sh file not found."
    exit 1
fi

# Find an LHE file corresponding to this seed
# Assuming LHE files are named with seed pattern, e.g., sample_${SEED}.lhe or similar
# Try common patterns
LHE_FILE=""
for pattern in "sample_${MY_SEED}.lhe" "events_${MY_SEED}.lhe" "lhe_${MY_SEED}.lhe" "*_${MY_SEED}.lhe"; do
    FOUND=$(find "$LHE_SOURCE_DIR" -name "$pattern" | head -1)
    if [ -n "$FOUND" ]; then
        LHE_FILE="$FOUND"
        break
    fi
...
# If not found with seed pattern, try to get any LHE file (for testing)
if [ -z "$LHE_FILE" ]; then
    echo "Warning: No LHE file found with seed pattern, looking for any .lhe file..."
    LHE_FILE=$(find "$LHE_SOURCE_DIR" -name "*.lhe" | head -1)
fi

if [ -z "$LHE_FILE" ] || [ ! -f "$LHE_FILE" ]; then
    echo "Error: No LHE file found in $LHE_SOURCE_DIR for seed $MY_SEED"
    exit 1
fi

echo "Using LHE file: $LHE_FILE"

# Export archive directory if set
if [ -n "$LHE_ARCHIVE_DIR" ]; then
    export LHE_ARCHIVE_DIR
fi

# Load the cmssw-el7 container and run shower workflow
cmssw-el7 --command-to-run "bash -x scripts/shower_existing_lhe.sh -l $LHE_FILE -s $MY_SEED"
...
echo "Shower workflow completed successfully with seed $MY_SEED."
...
cp "$MERGED_GENSIM" \
    /eos/user/c/chiw/JpsiJpsiPhi/MC_samples/GENSIM/DPS-JpsiJpsi-Phi/filter_JPsi_PtMin6p0_Phi_PtMin6p0/DPS-JpsiJpsi-Phi1020_JJPhi_4Mu2K_13p6TeV_TuneCP5_pythia8_Run3Summer22_GENSIM_shower_${MY_SEED}.root
